...
@section('content')
    ...
    <div class="bg-light py-4">
        <div class="container">
            <h5 class="header-style">@lang('web::auth.inbox') {{-- Inbox --}}</h5>
        </div>
    </div>
            ...
            <div class="card-header">
                <div class="gap-2">
                    <div style="float: left">
                        <h6 class=" mt-2 float-left">@lang('web::auth.inbox') {{-- Inbox --}}</h6>
                    </div>
                        ...
                        <div class="table-responsive">
                            ...
                            <table id="data-table-user" class="table mt-2" style="width:100%;font-size: 12px;">
                                <thead class="table-dark">
                                    <tr>
                                        <th style="text-align: left;">@lang('web::auth.bil') {{-- Bil --}}</th>
                                        <th style="text-align: left;">@lang('web::auth.from') {{-- Daripada --}}</th>
                                        <th style="text-align: left;">@lang('web::auth.recipient') {{-- Penerima --}}</th>
                                        <th style="text-align: left;">@lang('web::auth.subject') {{-- Subjek --}}</th>
                                        <th style="text-align: left;">@lang('web::auth.date') {{-- Tarikh --}}</th>
                                        <th style="text-align: left;">@lang('web::auth.action') {{-- Tindakan --}}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php $bil=1;?>
                                        @foreach($inbox as $key =>$value)
                                            <tr>
                                                <td>{{ $bil++}}</td>
                                                <td>{{ data_get($value,'from.name')}}</td>
                                                <td>{{ data_get($value,'to.name')}}</td>
                                                <td>
                                                    <a href="javascript:;" onclick="return funcPass('{{$value->id}}');"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#password-modal-preview">
