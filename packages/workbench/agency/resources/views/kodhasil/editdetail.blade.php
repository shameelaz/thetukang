...
<div class="container my-5">
    ...
    <div class="card style-border">
        ...
        {!! form()->open()->post()->action(url('/ptj/kodhasil/updatedetail'))->attribute('id', 'myform')->horizontal() !!}

        <input type="hidden" name="skhid" value="{{ data_get($servicekodhasildetail,'fk_service_kod_hasil') }}"/>
        <input type="hidden" name="kdhdid" value="{{ data_get($kdhd,'id') }}"/>
        ...
        <div id="div-individu" style="">
                ...
                <div class="col-sm-10">
                    <select class="form-select" id="" name="" disabled>
                        ...
                        @foreach($khidmat as $hk => $hv)
                            <option value="{{$hv->id}}" <?php if($hv->id == $kdhd->fk_lkp_perkhidmatan){echo "selected";}?> > {{ $hv->name }} </option>
                        @endforeach
                </select>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <select class="js-example-basic-single1" id="fk_kod_hasil" name="fk_kod_hasil" required="required" style="width: 100%">
                            <option value=""> Sila Pilih</option>
                            @foreach($hasil as $ck => $cv)
                                <option value="{{$cv->id}}"<?php if($cv->id == $servicekodhasildetail->fk_kod_hasil){echo "selected";}?> > {{ $cv->name }} </option>
                            @endforeach
                    </select>
                </div>
            </div>
