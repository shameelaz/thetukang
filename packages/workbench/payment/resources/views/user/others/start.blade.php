        ...
        <div id="wizard" class="card wizard style-border">
            ...
            <div class="content clearfix tab-content">
                <div class="tab-pane fade show active" id="pills-search" role="tabpanel" aria-labelledby="pills-home-tab">
                    <section>
                        {!! form()->open()->post()->action(url('/login/bayaran/hasil/save'))->attribute('id', 'formsubmit')->horizontal() !!}


                        <!-- <input type="hidden"  name="srvratemgt" value=" data_get($srvratemgt,'id') }}" /> -->
                        <div class="content-wrapper">
                            <h4 class="section-heading">Maklumat Hasil : {{ data_get($lkpperkhidmatan,'name') }}</h4>
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-12">
                                    <label for="lastName" class="sr-only">Kategori Perkhidmatan</label>
                                    <select class="form-select" id="srvratemgt" name="srvratemgt" required="required">
                                        <option value=""> Sila Pilih</option>
                                        @foreach($srvratemgt as $key => $value)
                                        <option value="{{ $value->id }}"> {{ data_get($value,'ptj.name') }} :{{ data_get($value,'lkpperkhidmatan.name') }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <!-- <h4 class="section-heading">Senarai Hasil</h4>
                            <div class="row">
...
@push('script')
    ...
    <script type="text/javascript">
        ...
        {
            $('select[name="srvratemgt"]').change(function()
            {
                var val = $(this).val();
                // console.log(val);
                ...
                if(val)
                {
                    ...
                    {
                        type: "GET",
                        url: "{{ URL::to('/login/bayaran/tiket/servicerate/search')}}"+"/"+val,
