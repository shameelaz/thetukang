                ...
                <div class="content-wrapper">
                    ...
                    <!-- foreach($list[0]->fk_service as $key => $list) -->
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-9">
                              <input type="text" readonly class="form-control-plaintext" id="agency" value="{{ data_get($list, '0.fkservice.fkkodhasil.description') }}">
                            </div>
                        </div>
                        <div class="mb-3 row">
                            <label for="agency" class="col-sm-3 col-form-label">PTJ</label>
                            <div class="col-sm-9">
                              <input type="text" readonly class="form-control-plaintext" id="ptj" value="{{ data_get($list, '0.fkservice.fkkodhasil.ptj.name') }}">
                            </div>
                        </div>
                        <div class="mb-3 row">
                            <label for="service" class="col-sm-3 col-form-label">Perkhidmatan</label>
                            <div class="col-sm-9">
                              <input type="text" readonly class="form-control-plaintext" id="service" value="{{ data_get($list, '0.fkservice.fkkodhasil.lkpperkhidmatan.name') }}">
                            </div>
                        </div>
                        <div class="mb-3 row">
                            <label for="codehasil" class="col-sm-3 col-form-label">Kod Hasil</label>
                            <div class="col-sm-9">
                              <input type="text" readonly class="form-control-plaintext" id="codehasil" value="{{ data_get($list, '0.fkservice.fkkodhasil.name') }}">
                            </div>
                        </div>
                        <div class="mb-3 row">
                                ...
                                <table id="data-table-agency" class="table mt-2" style="width:100%;font-size: 12px;">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            ...
                                            @foreach($list->where('flag', '=', $request->troliflag) as $key => $value)
                                                <tr>
                                                    <td class="text-center" valign="top">{{ $bil++ }}</td>
                                                    <td class="text-left" valign="top">{{ data_get($value, 'fkservice.serviceratemgt.lkpperkhidmatan.name') }}</td>
                                                    <td class="text-center" valign="top">{{ data_get($value, 'fkpayer.account_no') }}</td> <!-- hold utk bil -->
                                                    <td class="text-left" valign="top">{{ data_get($value, 'fkpayer.name') }}</td>
                                                    <td class="text-left" valign="top">{{ data_get($value, 'fkpayer.fkagency.name') }}</td>
